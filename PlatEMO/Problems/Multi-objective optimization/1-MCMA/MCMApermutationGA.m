function Offspring = MCMApermutationGA(Parent1,Parent2,proC)
...
% to include GA operator version for Many-Core
    ...
    for i = 1 : N/2
        ...
        %Diff_1   = setdiff(Parent1(i,:),ParentDec(i,1:k(i)),'stable');
        ...
        Offspring(i,:) = [ParentDec(i,1:k(i)) ParentDec(i+N/2,k(i)+1:end)];
        Offspring(i+N/2,:) = [ParentDec(i+N/2,1:k(i)) ParentDec(i,k(i)+1:end)];
        %Offspring(i+N/2,k(i)+1:end) = Diff_2(1,1:D-k(i));
        %Offspring(i+N/2,k(i)+1) = Diff_2(1,1);
