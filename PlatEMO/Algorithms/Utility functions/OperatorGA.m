...
%--------------------------------------------------------------------------
    ...
    Type      = arrayfun(@(i)find(Problem.encoding==i),1:7,'UniformOutput',false);
    ...
    if ~isempty(Type{7})        % Permutation variables parar aqui
        ...
        Offspring(:,Type{7}) = GApermutation(Parent1(:,Type{7}),Parent2(:,Type{7}),Problem);
    end
    if evaluated
